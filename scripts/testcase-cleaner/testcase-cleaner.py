    ...
    try:
        ...
        if hasattr(response, "usage_metadata"):
            ...
            if pricing_info:
                ...
                input_cost = (input_tokens / 1000000) * pricing_info.get("input", 0)
                reasoning_cost = (reasoning_tokens / 1000000) * pricing_info.get(
                    "output", 0
                )
                output_cost = (output_tokens / 1000000) * pricing_info.get("output", 0)
        ...
        if response.candidates and response.candidates[0].content.parts:
            ...
            print(
                f"[SUCCESS] Finished {input_file_path.name}. "
                f"Tokens (In: {input_tokens}, Reasoning: {reasoning_tokens}, Out: {output_tokens}). "
                f"Cost: ${file_cost:.6f}"
            )
        else:
            print(
                f"[WARNING] No valid content for {input_file_path.name}. Skipped. "
        ...
        print(
            f"[ERROR] An unexpected error occurred while processing file {input_file_path.name}. Reason: {e}"
        )


def main():
    parser = argparse.ArgumentParser(
        description="Concurrently process files and log API usage cost."
    )
    parser.add_argument(
        "input_folder", type=str, help="Path to the folder containing input files."
    )
    parser.add_argument(
        "output_folder",
        type=str,
    ...
    if not all([service_account_json_str, gcp_location]):
        print(
            "[ERROR] SERVICE_ACCOUNT_JSON and GCP_LOCATION must be set in the .env file."
        )
        return
    ...
    if not pricing_info:
        print(
            f"[WARNING] Pricing information for model '{model_name}' not found. Cost will not be calculated."
        )
    ...
    try:
        ...
        print(
            f"[INFO] Initializing Vertex AI for project '{project_id}' in location '{gcp_location}'..."
        )
        vertexai.init(
        ...
        generation_config = GenerationConfig(
            temperature=0, max_output_tokens=60000, seed=0, top_p=0.1
        )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.parallel) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_file):
            try:
                ...
                print(
                    f"[ERROR] An exception was generated for file {file_path.name}: {exc}"
                )
